		...
		Promise.all([
			...
			caches
				...
				.then(async (cache) => {
					...
					const essentialUrls = ['/', '/offline', '/manifest.json'];
					...
					for (const url of essentialUrls) {
						try {
							await cache.add(url);
							console.log(
								`Successfully cached essential: ${url}`
							);
						} catch (error) {
							console.warn(
								`Failed to cache essential ${url}:`,
								error
							);
						}
					}
					...
					try {
						...
						if (homeResponse.ok) {
							...
							const additionalJsMatches =
								homeText.match(
									/\/_next\/static\/chunks\/framework[^"']*\.js/g
								) || [];
							const webpackMatches =
								homeText.match(
									/\/_next\/static\/chunks\/webpack[^"']*\.js/g
								) || [];
							const mainMatches =
								homeText.match(
									/\/_next\/static\/chunks\/main[^"']*\.js/g
								) || [];
							...
							for (const assetUrl of assetUrls) {
								try {
									...
									console.log(
										`Successfully cached discovered asset: ${assetUrl}`
									);
								} catch (error) {
									console.warn(
										`Failed to cache asset ${assetUrl}:`,
										error
									);
								}
							}
							...
							try {
								const pageJsMatches =
									homeText.match(
										/\/_next\/static\/chunks\/pages\/[^"']+\.js/g
									) || [];
								for (const pageJs of pageJsMatches) {
									try {
										await cache.add(pageJs);
										console.log(
											`Successfully cached page JS: ${pageJs}`
										);
									} catch (error) {
										console.warn(
											`Failed to cache page JS ${pageJs}:`,
											error
										);
									}
								}
						...
						console.warn(
							'Failed to dynamically discover assets:',
							error
						);
					}
				}),
